...
class CardGenerator:
    ...
    def get_cards_by_rarity(
            self, rarity: str) -> Dict[str, List[Dict[str, Any]]]:
        """
        Get all cards of a specific rarity.

        Args:
            rarity: The rarity to filter
            by ("Common", "Uncommon", "Rare", "Legendary")

        Returns:
            Dictionary with card types
            as keys and lists of matching cards as values
        """
    ...
    def get_cards_by_cost(
            self, max_cost: int) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        Returns:
            Dictionary with card types as
            keys and lists of matching cards as values
        """
        result = {"creatures": [], "spells": [], "artifacts": []}
    ...
    def generate_random_deck(
            self, deck_size: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def list_available_cards(self) -> None:
        ...
        print("Creatures:")
        for creature in self._creatures:
            print(f"  - {creature['name']} ({creature['cost']} "
                  f"mana, {creature['rarity']})")
        ...
        print("\nArtifacts:")
        for artifact in self._artifacts:
            print(f"  - {artifact['name']} ({artifact['cost']} mana,"
                  f" {artifact['rarity']})")
...
def main() -> None:
    ...
    print(f"\nRandom Deck (5 cards): "
          f"{[card['name'] for card in generator.generate_random_deck(5)]}")
